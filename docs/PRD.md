...
| **AI 引擎** | 服务器端配置 | **多模型支持 (Gemini/DeepSeek等)** | 支持用户自定义 Key |
| **全文搜索** | 基础搜索 | **FTS5 本地全文索引** | 支持毫秒级全书检索 |
...
#### 3.5.1 多模型 AI 支持
**需求描述**：
- **广泛兼容**：支持 Google Gemini, OpenAI, Claude, DeepSeek, Qwen 等主流大模型。
- **自定义接入**：支持兼容 OpenAI 格式的自定义 API Endpoint。
- **一键切换**：用户可在设置中配置多个供应商，并快速切换当前活跃模型。

#### 3.5.2 本地全文检索 (FTS5)
**需求描述**：
- **技术实现**：基于 SQLite FTS5 扩展实现。
- **功能体验**：支持全书内容的关键词检索，即时返回包含上下文的搜索结果。
- **AI 增强**：AI 助手可利用全文索引进行跨章节知识检索和问答（RAG 基础版）。

#### 3.5.3 智能生词复习
**需求描述**：
- **优先级算法**：后台自动计算生词优先级（基于查询频次、最后查询时间等）。
- **学习状态**：生词标记为 New / Learning / Mastered。
- **自动调度**：每日后台任务自动更新复习列表。
