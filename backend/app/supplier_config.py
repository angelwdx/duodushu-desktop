...
SUPPLIER_PRESETS: Dict[SupplierType, Dict] = {
    SupplierType.GEMINI: {
        ...
        "models": [
            ...
            ModelInfo(
                id="gemini-2.5-pro", name="Gemini 2.5 Pro", description="极高复杂任务处理能力", context_length=2000000
            ),
            ModelInfo(
                id="gemini-2.5-flash", name="Gemini 2.5 Flash", description="平衡性能与响应速度", context_length=1000000
            ),
        ],
    ...
    SupplierType.OPENAI: {
        ...
        "models": [
            ModelInfo(
                id="gpt-5", name="GPT-5", description="最新开发模型，专为编码和AI代理优化", context_length=200000
            ),
            ModelInfo(id="gpt-5.2", name="GPT-5.2", description="企业级模型，处理复杂知识工作", context_length=200000),
            ModelInfo(id="gpt-4o", name="GPT-4o", description="成熟稳定的模型（2026年2月退役）", context_length=128000),
        ],
    ...
    SupplierType.QWEN: {
        ...
        "models": [
            ...
            ModelInfo(id="qwen3-235b-a22b", name="Qwen3-235B-A22B", description="高性能模型", context_length=32000),
            ModelInfo(id="qwen3-coder-480b", name="Qwen3-Coder-480B", description="专用编码模型", context_length=32000),
        ],
...
class MultiSupplierConfig(BaseModel):
    ...
    def get_active_supplier(self) -> Optional[SupplierConfig]:
        ...
        if self.active_supplier and self.active_supplier in self.suppliers:
            # 确保 is_active 状态与 active_supplier 同步
            supplier_config = self.suppliers[self.active_supplier]
            if not supplier_config.is_active:
                supplier_config.is_active = True
...
def migrate_old_config(old_config: dict) -> MultiSupplierConfig:
    ...
    if api_keys.get("gemini_api_key"):
        new_config.add_or_update_supplier(
            SupplierConfig(
